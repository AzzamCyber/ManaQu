...
import { Plus, X, ArrowUpFromLine, AlertCircle } from 'lucide-react';
...
export default function OutboundClient({ initialData, items, gudangList }: { initialData: Transaksi[], items: Barang[], gudangList: Gudang[] }) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append('jenis', 'OUT');
    
    // Validasi stok di client-side
    ...
    if (selectedItem && jumlah > selectedItem.jumlah_stok) {
      setError(`Stok tidak mencukupi! Sisa stok ${selectedItem.nama_barang}: ${selectedItem.jumlah_stok}`);
      setIsLoading(false);
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex justify-end">
        <button onClick={() => { setError(null); setIsModalOpen(true); }}
          className="px-6 py-3 bg-[#f87171] hover:bg-[#ef4444] text-black font-black text-sm uppercase border-2 border-black shadow-[4px_4px_0_0_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_#000] flex items-center transition-all">
          <Plus className="w-5 h-5 mr-2" strokeWidth={3} />Catat Outbound
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#fef08a] border-b-2 border-black text-black font-black text-xs tracking-widest uppercase">
              <tr>
                ...
                <th className="px-6 py-5">Tujuan / Penerima</th>
            ...
            <tbody className="divide-y-2 divide-black text-black font-bold text-sm">
              {initialData.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-16 text-center bg-gray-50">
                  <div className="flex flex-col items-center"><ArrowUpFromLine className="w-12 h-12 mb-4 text-black opacity-50" strokeWidth={2} /><p className="text-lg font-black uppercase">Belum ada barang keluar</p></div>
                </td></tr>
              ) : (
                initialData.map((trx) => (
                  <tr key={trx.id} className="hover:bg-[#fef08a] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-black uppercase">{trx.tujuan || '-'}</div>
                      <div className="text-[10px] uppercase mt-1">Penerima: {trx.penerima || '-'}</div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-lg font-black text-[#ef4444]">-{trx.jumlah}</span>
                    </td>
                    <td className="px-6 py-4 uppercase text-xs">{trx.gudang ? trx.gudang.nama : '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white border-2 border-black shadow-[8px_8px_0_0_#000] w-full max-w-xl">
            <div className="px-8 py-5 border-b-2 border-black bg-[#f87171] flex justify-between items-center">
              <h3 className="text-xl font-black text-black uppercase tracking-tight">Catat Barang Keluar</h3>
            ...
            <form onSubmit={onSubmit} className="p-8 space-y-6">
              ...
              <div>
                ...
                <select name="barangId" required className="w-full px-4 py-3 text-sm font-bold bg-[#f8fafc] border-2 border-black focus:outline-none focus:bg-[#e0f2fe] focus:shadow-[4px_4px_0_0_#000] transition-all uppercase">
                  ...
                  {items.map(item => <option key={item.id} value={item.id} disabled={item.jumlah_stok === 0}>{item.nama_barang} (Stok: {item.jumlah_stok}){item.jumlah_stok === 0 && ' - KOSONG'}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div><label className="block text-xs font-black text-black uppercase mb-2">Dari Gudang</label>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div><label className="block text-xs font-black text-black uppercase mb-2">Tujuan Pengiriman</label><input type="text" name="tujuan" className="w-full px-4 py-3 text-sm font-bold bg-[#f8fafc] border-2 border-black focus:outline-none focus:bg-[#e0f2fe] focus:shadow-[4px_4px_0_0_#000] transition-all uppercase" placeholder="KOTA/CABANG/PROYEK" /></div>
                <div><label className="block text-xs font-black text-black uppercase mb-2">Nama Penerima</label><input type="text" name="penerima" className="w-full px-4 py-3 text-sm font-bold bg-[#f8fafc] border-2 border-black focus:outline-none focus:bg-[#e0f2fe] focus:shadow-[4px_4px_0_0_#000] transition-all uppercase" placeholder="NAMA PENERIMA" /></div>
              </div>
              <div><label className="block text-xs font-black text-black uppercase mb-2">Keterangan (Opsional)</label><input type="text" name="keterangan" className="w-full px-4 py-3 text-sm font-bold bg-[#f8fafc] border-2 border-black focus:outline-none focus:bg-[#e0f2fe] focus:shadow-[4px_4px_0_0_#000] transition-all uppercase" placeholder="CATATAN..." /></div>
              <div className="pt-8 border-t-2 border-black flex justify-end space-x-4">
                ...
                <button type="submit" disabled={isLoading} className="px-8 py-3 bg-[#f87171] border-2 border-black text-black text-sm font-black uppercase shadow-[4px_4px_0_0_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_#000] transition-all disabled:opacity-50">{isLoading ? 'Wait...' : 'Simpan Outbound'}</button>
              </div>
            </form>
          </div>
        </div>
