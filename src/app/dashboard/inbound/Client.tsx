...
import { Plus, X, ArrowDownToLine, AlertCircle } from 'lucide-react';
...
export default function InboundClient({ initialData, items, gudangList }: { initialData: Transaksi[], items: Barang[], gudangList: Gudang[] }) {
    ...
    <div className="space-y-6">
      <div className="flex justify-end">
        <button onClick={() => { setError(null); setIsModalOpen(true); }}
          className="px-6 py-3 bg-[#34d399] hover:bg-[#10b981] text-black font-black text-sm uppercase border-2 border-black shadow-[4px_4px_0_0_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_#000] flex items-center transition-all">
          <Plus className="w-5 h-5 mr-2" strokeWidth={3} />Catat Inbound
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#fef08a] border-b-2 border-black text-black font-black text-xs tracking-widest uppercase">
              <tr>
                ...
                <th className="px-6 py-5">Supplier / PO</th>
            ...
            <tbody className="divide-y-2 divide-black text-black font-bold text-sm">
              {initialData.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-16 text-center bg-gray-50">
                  <div className="flex flex-col items-center"><ArrowDownToLine className="w-12 h-12 mb-4 text-black opacity-50" strokeWidth={2} /><p className="text-lg font-black uppercase">Belum ada barang masuk</p></div>
                </td></tr>
              ) : (
                initialData.map((trx) => (
                  <tr key={trx.id} className="hover:bg-[#fef08a] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-black uppercase">{trx.supplier || '-'}</div>
                      <div className="text-[10px] uppercase mt-1">PO: {trx.noPO || '-'}</div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="text-lg font-black text-[#10b981]">+{trx.jumlah}</span>
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white border-2 border-black shadow-[8px_8px_0_0_#000] w-full max-w-xl">
            <div className="px-8 py-5 border-b-2 border-black bg-[#34d399] flex justify-between items-center">
              <h3 className="text-xl font-black text-black uppercase tracking-tight">Catat Barang Masuk</h3>
            ...
            <form onSubmit={onSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div><label className="block text-xs font-black text-black uppercase mb-2">Lokasi Gudang</label>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div><label className="block text-xs font-black text-black uppercase mb-2">Nama Supplier (Opsional)</label><input type="text" name="supplier" className="w-full px-4 py-3 text-sm font-bold bg-[#f8fafc] border-2 border-black focus:outline-none focus:bg-[#e0f2fe] focus:shadow-[4px_4px_0_0_#000] transition-all uppercase" placeholder="PT SUKSES MAKMUR" /></div>
                <div><label className="block text-xs font-black text-black uppercase mb-2">Nomor PO (Opsional)</label><input type="text" name="noPO" className="w-full px-4 py-3 text-sm font-bold bg-[#f8fafc] border-2 border-black focus:outline-none focus:bg-[#e0f2fe] focus:shadow-[4px_4px_0_0_#000] transition-all uppercase" placeholder="PO-2026-001" /></div>
              </div>
              ...
              <div className="pt-8 border-t-2 border-black flex justify-end space-x-4">
                ...
                <button type="submit" disabled={isLoading} className="px-8 py-3 bg-[#34d399] border-2 border-black text-black text-sm font-black uppercase shadow-[4px_4px_0_0_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_#000] transition-all disabled:opacity-50">{isLoading ? 'Wait...' : 'Simpan Inbound'}</button>
              </div>
            </form>
          </div>
        </div>
